...
# -*- coding: utf-8 -*-
"""
قراءة شاملة لأول 2000 سجل من بيانات OpenFDA
"""
import json
...
print(f'📖 سنقرأ أول 2000 سجل بالتفصيل...\n')
print('='*120)
...
sample_display = []
for i, record in enumerate(results[:2000]):
    ...
    # Save interesting samples (first 20)
    if len(sample_display) < 20:
        if dosage_admin and len(dosage_admin[0]) > 200:  # Only substantial dosage text
            sample_display.append({
                'index': i,
                'substance': substance[0] if substance else 'N/A',
...
print(f'\n📈 إحصائيات شاملة لأول {stats["total_examined"]} سجل:\n')
...
print(f'\n\n{"="*120}')
print(f'🔍 عينة من السجلات الكاملة (20 سجل مع نصوص جرعات جيدة):')
print(f'{"="*120}\n')

for idx, sample in enumerate(sample_display):
    print(f'\n### سجل رقم {sample["index"]} (عينة #{idx+1}) ###')
    print(f'المادة الفعالة: {sample["substance"][:100]}')
    print(f'الاسم العام: {sample["generic"][:100]}')
    print(f'الاسم التجاري: {sample["brand"][:100]}')
...
print(f'\n\n✅ تم الانتهاء من قراءة وتحليل أول {stats["total_examined"]} سجل')
