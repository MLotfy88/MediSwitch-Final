...
'Rectal': r'\b(?:rectal|rectally|suppository)\b',
    ...
    def extract_full_dosage_info(self, record: Dict) -> Dict:
        ...
        if 'contraindication' in text_lower and len(text) > 50:
            result['contraindications'] = text.strip()
        
        if 'warning' in text_lower and len(text) > 50:
        ...
        if 'precaution' in text_lower and len(text) > 50:
            result['precautions'] = text.strip()
        
        if 'adverse reaction' in text_lower or 'side effect' in text_lower:
            if len(text) > 50:
                result['adverse_reactions'] = text.strip()
        
        if 'overdosage' in text_lower or 'overdose' in text_lower:
            if len(text) > 50:
...
def update_database_enhanced():
    ...
    for i, spl_file in enumerate(spl_files[:2], 1):
