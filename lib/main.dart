    ...
    try {
      ...
      try {
        ...
        if (count == null || count == 0) {
          ...
          final foodSamples = await db.rawQuery(
            'SELECT trade_name FROM food_interactions LIMIT 5',
          );
          logger.i(
            "main: Sample food_interactions trade_names: ${foodSamples.map((e) => '"${e['trade_name']}"').toList()}",
          );

          if (foodSamples.isNotEmpty) {
            final firstFoodDrug = foodSamples.first['trade_name'] as String;
            logger.i(
              "main: Attempting match for first food drug: '$firstFoodDrug'",
            );
          ...
          final ingCountRes = await db.rawQuery(
            'SELECT COUNT(*) as count FROM med_ingredients',
          );
          ...
          if (ingCount == 0) {
            logger.w(
              "main: med_ingredients is EMPTY. High risk drugs will definitely fail.",
            );
          }
