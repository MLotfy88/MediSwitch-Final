...
class DosageCalculatorService {
  ...
  _Concentration? _parseConcentration(String tradeName, double defaultStrength) {
    // Regex to find patterns like "120mg/5ml", "250 mg / 5 ml", "100mg/ml"
    // Group 1: Strength (e.g., 120)
    // Group 3: Volume (e.g., 5), optional
    final RegExp regex = RegExp(r'(\d+(?:\.\d+)?)\s*mg\s*/\s*(\d+(?:\.\d+)?)?\s*ml', caseSensitive: false);
    final match = regex.firstMatch(tradeName);

    if (match != null) {
      final double strength = double.tryParse(match.group(1) ?? '') ?? defaultStrength;
      final double volume = double.tryParse(match.group(2) ?? '1') ?? 1.0; // Default to 1ml if not specified
    ...
    if (defaultStrength > 0) {
       ...
       return _Concentration(strength: defaultStrength, volume: 1.0); // Fallback to 1ml if only strength is known?
    }
  ...
  DosageResult _calculateParacetamolDosage(
    ...
    if (formLower.contains("tablet") || formLower.contains("tab") || formLower.contains("أقراص") || formLower.contains("قرص")) {
      if (age >= 12) {
        dosage =
        ...
        final double tabletStrength = medicine.concentration;
        if (tabletStrength <= 0) {
          return DosageResult(
    ...
    } else if (formLower.contains("syrup") || formLower.contains("susp") || formLower.contains("شراب") || formLower.contains("معلق")) {
      
      final parsed = _parseConcentration(medicine.tradeName, medicine.concentration);
      
      if (parsed == null || parsed.strength <= 0) {
        return DosageResult(
          dosage: "تعذر تحديد تركيز الدواء من الاسم (${medicine.tradeName}).",
          warning: "يرجى التأكد من أن اسم الدواء يحتوي على التركيز (مثلاً 120mg/5ml).",
        );
      }
      ...
      notes = "التركيز المستخدم: ${parsed.strength.toStringAsFixed(0)}مجم / ${parsed.volume.toStringAsFixed(0)}مل";
       ...
       dosage = "للأطفال ${age} سنوات: ${_roundToNearestHalf(minMl).toStringAsFixed(1)}-${_roundToNearestHalf(maxMl).toStringAsFixed(1)} مل (${(minMl*20).toStringAsFixed(0)}-${(maxMl*20).toStringAsFixed(0)} نقطة تقريباً) كل 4-6 ساعات.";
    } else {
      return DosageResult(
  ...
  DosageResult _calculateIbuprofenDosage(
    ...
    if (formLower.contains("tablet") || formLower.contains("tab") || formLower.contains("أقراص") || formLower.contains("قرص")) {
      ...
      if (parsed == null || parsed.strength <= 0) {
        return DosageResult(
          dosage: "تعذر تحديد تركيز الدواء من الاسم (${medicine.tradeName}).",
          warning: "يرجى التأكد من أن اسم الدواء يحتوي على التركيز (مثلاً 100mg/5ml).",
        );
      }
  ...
  DosageResult _calculateAmoxicillinDosage(
    ...
    final double dailyDoseMg = weight * 30; // Using 30mg/kg/day as a safe effective average
    final double singleDoseMg = dailyDoseMg / 3; // Dose per administration (every 8h)
    String dosage = "";
    String? warning;
    String? notes;
    ...
    if (formLower.contains("capsule") || formLower.contains("tab") || formLower.contains("كبسولة") || formLower.contains("قرص")) {
      if (age >= 12) {
        dosage =
      ...
      if (parsed == null || parsed.strength <= 0) {
        return DosageResult(
          dosage: "تعذر تحديد تركيز الدواء من الاسم (${medicine.tradeName}).",
          warning: "يرجى التأكد من أن اسم الدواء يحتوي على التركيز (مثلاً 250mg/5ml).",
        );
      }
