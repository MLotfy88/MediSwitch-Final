...
def find_ddinter_id(ddinter_index: Dict, ingredient: str) -> str:
    ...
    Strategy: 1) Exact match, 2) Synonym lookup, 3) Normalized exact match
    """
    # Try synonym dictionary first
    synonym = DRUG_SYNONYMS.get(ingredient.lower().strip())
    if synonym:
        # Look up the synonym in DDInter
        syn_normalized = normalize_ingredient(synonym)
        if syn_normalized in ddinter_index:
            return ddinter_index[syn_normalized]
        if synonym.lower().strip() in ddinter_index:
            return ddinter_index[synonym.lower().strip()]
...
def process_pipeline():
    print("🚀 Enhanced Enrichment Pipeline v3 (Curated Synonyms)\n")
    print("=" * 70)
    ...
    print(f"   Loaded {len(DRUG_SYNONYMS):,} curated synonyms\n")
    ...
    for i_med, med in enumerate(local_meds):
        ...
        for ingredient in ingredients:
            # Check if it's in synonym dict
            if ingredient.lower().strip() in DRUG_SYNONYMS:
