...
def fetch_food_interactions(conn, ddinter_id: str) -> List[Dict]:
    """Fetch drug-food interactions."""
    try:
        sql = """
            SELECT food_name, severity, description, management
...
def process_pipeline():
    print("🚀 Enhanced Enrichment Pipeline v4 (Food Interactions)\n")
    print("=" * 70)
    ...
    processed_pairs = set()
    processed_food_pairs = set() # (med_id, food_name)
    ...
    for i_med, med in enumerate(local_meds):
        ...
        if (i_med + 1) % 2000 == 0:
            print(f"   Processed {i_med + 1:,} meds | DDI: {len(all_rules):,} | DFI: {len(all_food_interactions):,}", flush=True)

    conn_ddinter.close()
    
    # Save DDI Rules (Chunked)
    print(f"\n💾 Saving DDI Rules...")
    ...
    # Save Food Interactions (Single File - smaller dataset)
